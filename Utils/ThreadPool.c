...
void ThreadPoolQueueDataCB(void *data, size_t size, DataType type){
    ThreadPoolTask *task = (ThreadPoolTask *)data;
    if((task != NULL) && (task->cb != NULL)) {
        task->cb(task->arg);
    }
}
...
static void *thread_routine(void *arg)
{
    ...
    while(1) {
        ...
        QueuePop(&g_pool_manager.queue, ThreadPoolQueueDataCB);
        pthread_mutex_unlock(&g_pool_manager.queue_lock);
    }
    return NULL;
}
...
void ThreadPoolAddTask(TaskWorkCB cb, void *arg)
{
    ...
    pthread_mutex_lock(&g_pool_manager.queue_lock);
    QueuePush(&g_pool_manager.queue, &task, sizeof(ThreadPoolTask), DataTypeStruct);
    pthread_mutex_unlock(&g_pool_manager.queue_lock);
